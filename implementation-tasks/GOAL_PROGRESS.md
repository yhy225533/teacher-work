...
## 2026-09-06 10:48 · V18-A · DONE

- 关键改动：新建 `src/shared/feedback-contracts.ts`（FEEDBACK_TRANSCRIPT_MAX_CHARS=30000 / FEEDBACK_MAX_TOKENS=1500 / FEEDBACK_PROMPT_VERSION='v18-01-v1' / FEEDBACK_BODY_MAX_CHARS=8000 + TranscriptResult / GenerateFeedbackRequest / GeneratedFeedback / FeedbackNoteMetadata 及全部守卫）；ipc-contracts 追加 FEEDBACK_IPC_CHANNELS（readTranscript / generate）、FEEDBACK_ERROR 错误码与 IpcChannel 联合；preload-api / preload 增加 feedback 命名空间；新建 `src/main/feedback/feedback-service.ts`（readTranscript：dialog 注入选 .txt/.md → UTF-8 读 → 头截 + truncated，不写 files/不复制/不索引/正文不进日志；generate：getDraftLessonSnapshot 校验 + skill prompt 或内置默认三段提示词 + 学生/课程/课次/反馈日期上下文 + 转写文本 → requestText 非流式 → 草稿不落库；resolveFeedbackDate = scheduled_at 本地日期 ?? 当天）；新建 `src/main/ipc/feedback-ipc.ts`（两条通道白名单/守卫/错误映射，AiGatewayError → AI_ERROR 透传）并接入 index.ts（含 txt/md dialog 注入与 before-quit 注销）；NoteRecord.aiMetadata 双轨（DraftNoteMetadata | FeedbackNoteMetadata），core-data mapNote 按双守卫解析，draft-scope 新增 draftNoteMetadata 收窄助手，draft-view-model/draft-panel/draft-service 按助手收窄（行为不变）。
- 修改文件：`src/shared/feedback-contracts.ts`（新）、`src/shared/ipc-contracts.ts`、`src/shared/core-contracts.ts`、`src/shared/preload-api.ts`、`src/preload/index.ts`、`src/main/feedback/feedback-service.ts`（新）、`src/main/ipc/feedback-ipc.ts`（新）、`src/main/index.ts`、`src/main/data/core-data-service.ts`、`src/main/draft/draft-service.ts`、`src/renderer/draft-scope.ts`、`src/renderer/draft-view-model.ts`、`src/renderer/draft-panel.tsx`、`tests/feedback-contracts.test.ts`（新）、`tests/feedback-service.test.ts`（新）、`tests/feedback-ipc.test.ts`（新）、`tests/core-data.test.ts`、`tests/draft-service.test.ts`、`tests/v17-a-draft-bank.test.ts`、`tests/v17-d-bank-selection.test.ts`、`tests/v1.5.3.1-scope-flow.test.ts`、`implementation-tasks/STATUS.md`、本文件。
- 验证命令与结果：`npm test` ✅ 82 files / 414 tests passed（1 skipped）；`npm run typecheck` ✅；`npm run lint` ✅。反馈链专项 18 例：守卫正反例（truncated/skillId 可选/body 上限/双轨 aiMetadata）、fake gateway prompt 注入（skill prompt、默认三段、学生/课程/课次/反馈日期上下文、1500 token 上限）、截断（30000 头截 + truncated 标注）、取消返回 null、AI_NOT_CONFIGURED 透传、跨课次学生拒绝、notes 表零行（草稿不落库）、IPC 白名单注册/注销、read-transcript 不写 files 表、错误日志不含转写正文。
- 人工/真实环境验证：无 UI 改动，无需窗口走查（V18-B 起补）。
...
## 2026-09-07 · V1.9 范围扩容：教学界面信息密度收口并入（方案冻结，plan 提交）

- 产品负责人三轮界面评审（2026-09-07，`tmp/mockups/` 示意本地不入库）：① 课件区加导出按钮后"非常非常挤"→ 方案 B（双头合并）入选；② 课程页右上方静态名片信息无用（"打开主要是为了知道现在做哪一课、有哪些内容"）→ 当前课次行动卡；③ 澄清主诉是**备课页"很乱很乱"**→ 对话式重做（撤模式切换器与固定步骤链；"但凡有东西，本质都是 AI 修改"）+ 修改记录默认收起（"要么不放在这，要么可以收回去隐藏"）；④ CodeMirror 6 提案评估后本人裁决"对我现在用处不大"（D60 候选缓议）。
- 拍板："先修改刚刚说的那个教学里的UI问题。写一下方案，也是在V1.9里一并做完的"——UI 重做并入 V1.9，不另开 V1.8.2。
- 方案：`docs/v1.9-teaching-ui-restructure-plan.md`（UI 设计基准）；决策 D55–D60 追加于 `implementation-tasks/V1_9_DECISIONS.md`（文件头同步改为双范围）——D55 备课工作台对话式重做（右栏=与 AI 对话、主区=单一舞台同卡替换、不编号不步骤链）；D56 修改记录默认收起 + 成果编辑复用 MdEditor 受控用法（消灭课件区 MdEditor vs 备课页裸 textarea 两套编辑体验）；D57 课件区双头合并（三层头部 11+ 按钮 → 单工具行 5 键，低频/高危入 ⋯ 收纳菜单）；D58 课程页当前课次行动卡（统计条退役、chips 全部 overview 派生）；D59 非连续聊天流（对话式呈现，生成/题库/发布合同零改动）；D60 CodeMirror 6 候选不进本期（触发条件：真实备课试跑确认编辑体验为主要摩擦点）。
