...
| V110-C 生成类型三按钮（问题 4） | TODO | new 模式恢复讲义/例题/作业三按钮并排（可连发独立文件），撤除单选下拉与 improveKind；drafts.generate 合同零改动；D63 |
| V110-D 最终门禁与验收 | TODO | 全量测试、typecheck、lint、build、diff check、隔离 Windows 冒烟（状态保活往返 + 批量移除 + 连续生成）+ docs/v1.10-acceptance.md；产品负责人走查确认后创建 checkpoint-V1.10-pass |
