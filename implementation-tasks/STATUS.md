...
| V110-A 备课状态保活（问题 5） | DONE | App.tsx picker 往返不卸载教学内容页（DraftPanel 勾选/要求/模式/流式状态保留）+ copyToLesson/importToLesson 补发既有 contentChanged 广播 + 钉测；设计基准 docs/v1.10-walkthrough-fixes-plan.md §2 / D61。2026-09-09 完成：picker 不切页改 overlay（教学内容页/DraftPanel 保持挂载，状态不丢）；external-library-ipc copyToLibrary/copyToLesson + file-ipc copyToLesson 三处补发既有 contentChanged；DraftPanel 新增 onContentChanged→reload 订阅；新增 v1.10-prep-state-keepalive 5 例 + 演进 2 处；全量 92 files / 526 tests（1 skip 既有）、typecheck、lint 全绿 |
| V110-B 资料移除可达性（问题 1） | DONE | 树节点 hover ✕ + 批量管理 toggle（勾选多份一次移除）+ 历史版本行移除入口；全部走既有 softDeleteFile，零新 IPC；readOnly 零入口、当前讲义当前版不出 ✕；D62。2026-09-09 完成：MaterialTreeNodeRow hover ✕（canRemove = onRemoveFile + removableFileIds 白名单，白名单 = 当前资料 − 当前讲义当前版）+ 树标题管理 toggle + 管理态勾选/lesson-manage-bar 批量串行 softDeleteFile（确认列清单）+ 历史版本行 ✕；⋯ 红区兜底保留；新增 v1.10-remove-entry-points 5 例 + 演进 material-library-ui 1 处；全量 93 files / 531 tests（1 skip 既有）、typecheck、lint 全绿 |
...
| V111-B PDF 应用内预览 | DONE | `npm i react-pdf`（pdfjs React 19 封装）+ PdfPreview 组件（Document/Page 连续滚动、Vite worker 配置）+ reader binary→PDF 分支接线 + 样式 + 失败态逃生门；门禁含 production build（worker 资产实跑验证）。2026-09-10 完成：react-pdf ^10.5.0 + PdfPreview（静态 import 经 renderer 边界规则；worker Vite 资产管道；ResizeObserver 自适应；双 layer 关闭无跳转面）+ reader binary→PDF 分支 + fallback 逃生门 + pdf-binary 解码工具 + DOMMatrix 测试占位（setup-node-dom-stubs，初版 lazy 方案被边界测试正确拒绝后改静态 + 占位）；新增 v1.11-pdf-preview 7 例；全量 97 files / 551 tests（550 + 1 skip 既有）、typecheck、lint、build（pdf.worker 资产产出）全绿 |
| V111-C Word(docx) 应用内预览 | DONE | `npm i docx-preview` + DocxPreview 组件（renderAsync 到受控容器、卸载清空）+ reader docx 分支接线 + 样式；.doc/.pptx/.xlsx 维持 unsupported（钉测明示）。2026-09-10 完成：docx-preview ^0.4.0 + DocxPreview（静态 import renderAsync、dataUrl→ArrayBuffer 复用 pdf-binary、受控容器卸载清空、inWrapper 版式、失败态 inline-error）+ reader DOCX_MIME 分支 + 同款逃生门 + 容器样式组；演进 v1.11-pdf-preview 至 10 例（docx 分支/静态导入/常量与 Main 白名单双重钉测/依赖白名单）；全量 97 files / 554 tests（553 + 1 skip 既有）、typecheck、lint 全绿 |
| V111-D 最终门禁与验收 | DONE | 2026-09-09 完成：全量 96 files / 553 tests + typecheck + lint + production build（worker 资产产出）+ diff check 全绿；隔离 Windows 冒烟 10/10（真实 PDF canvas 像素断言 + docx wrapper 文本 + 三分支零回归 + stderr，连跑 4 轮）；冒烟暴露并修复两处渲染缺陷（pdfjs worker 版本失配→深路径指 react-pdf 内嵌 v5 worker；file 载荷引用竞态→useMemo 记忆化）并补钉测；进程/临时目录零残留；`docs/v1.11-acceptance.md`；`checkpoint-V1.11-pass` 待产品负责人走查确认后创建 |

## V1.12 已立项（外部资料应用内预览 + docx 内嵌图片修复）

基线：V1.11 四节点已全部完成（V111-D 全量门禁 + 冒烟 10/10 + `docs/v1.11-acceptance.md`），`checkpoint-V1.11-pass` 待产品负责人走查确认。产品负责人 2026-09-10 实测反馈两问题并授权修复（解决一下）：①带内嵌图片的 docx 预览图片不显示（docx-preview 默认 blob: URL 被 CSP img-src 拦截）；②外部资料页无预览能力（V1.1 纯元数据面板，V1.11 预览只接了课次阅读器）。按方案合并实施为 **V1.12**（docx 图片修复并入本版第一节点，不单出 V1.11.1 标签）。设计基准 `docs/v1.12-external-preview-plan.md`，决策 D70–D72（`implementation-tasks/V1_12_DECISIONS.md`）。任务链 `implementation-tasks/v1.12-tasks/`，实施顺序 V112-A → B → C，同一时刻最多一个 `IN_PROGRESS`；V112-C 为唯一全量验收点（验收文档 `docs/v1.12-acceptance.md`）。零 migration、零新依赖；**新 IPC 仅一条 `external-library:read-preview`**（载荷通道，与 V1.9 export:get-print-payload 的 Main 组装单次响应模式同构；纯只读——不写 files 表、不进备份/索引、零 contentChanged、响应不含路径；路径安全复用 resolveEntry realpath + within-root）；CSP 零改动（docx 内嵌图改 data: URL 走既有 img-src data: 白名单）；外部资料原文件保持只读；范围不含 .doc/.pptx/.xlsx 渲染与外部资料 AI 参考线（后续候选）。不运行 portable/installer；`checkpoint-V1.12-pass` 待产品负责人按验收文档走查确认后创建。
...
## V1.14 已立项（课件区新建讲义 + 树降噪，D82–D87）

基线：V1.13.2 已完成（`b8c52ca`）。2026-09-14 产品负责人实测：①课件区无任何从零创建入口（⋯ 菜单全是加工已有文件的动作）——手写讲义路径断裂；②四组展示下空组长文案/重复树头/全员外部资料徽标造成结构性噪音。拍板：新建入口 = ⋯ 菜单 + 讲义组头 ＋ 双入口、新建形态 = 版本链讲义（创建 `名称 · 第 1 版.md`，重名自动顺延，创建后直进编辑器）、空组一行化、附加降噪三项全做（树头去重/隐藏外部资料徽标/启发式补词）。同日复审修订（交互示意 `docs/v1.14-ui-mock.html` 全量更新并逐场景验证）：**D86** 版本链按基名分链（修复 classify 单链假设——第二基名链头被误吞进历史块；链头行显示基名；主讲义=最近保存链头；hover ✕ 白名单扩至全部链头；历史版本收 ⋯「本文件」按需唤出、底部常驻块退役）+ **D87** 工具行重排（修改这份｜编辑｜导出 PDF｜⛶ 图标｜⋯，恢复 v1.9/D57 冻结原序）。设计基准 `docs/v1.14-lesson-doc-create-and-tree-declutter-plan.md`，决策 D82–D87（`implementation-tasks/V1_14_DECISIONS.md`）。任务链 `implementation-tasks/v1.14-tasks/`，顺序 V114-A → B → C；零 migration、零新依赖；新 IPC 仅 `files:create-lesson-doc` 一条；D46 语义零变化（新建产物与设为讲义底稿同构）。`checkpoint-V1.14-pass` 待走查确认后创建。
...
| V114-C 最终门禁与验收 | DONE | 全量门禁（102 files / 616 tests + typecheck + lint + build + git diff --check）+ 隔离冒烟 15/15 × 3 轮（新建→编辑→第 2 版版链 + 多链并立/历史按链唤出 + 工具行顺序/⛶ + 降噪断言 + 落库 + stderr；冒烟发现并修复「创建后直进编辑态被切文件 effect 反清」真实缺陷——enterEditingRef 修复 + 门禁复跑全绿）+ `docs/v1.14-acceptance.md`（2026-09-14） |

V1.14 全链完成；`checkpoint-V1.14-pass` 待产品负责人按 `docs/v1.14-acceptance.md` §5 走查清单确认后创建。
...
| 里程碑 | 状态 | 计划内容 |
|---|---|---|
| V1131-A 菜单可选中 + 遮挡避让 + 裁剪逃逸 + 卡顿修复 | DONE | 容器关闭判定 + fixed 定位/向上翻转 + ✕ 让位 + 管理态保留改组菜单 + MarkdownDocument memo；真实鼠标探针 7/7 + V113 冒烟 13/13 零回归 + 全量 101 files / 597 tests 全绿 |
