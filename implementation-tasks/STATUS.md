...
| V113-B 启发式分组纯模块 + 四组树 + 改组菜单 | IN_PROGRESS | lesson-material-groups.ts 规则表（D75 七条首中即停）+ LessonMaterialReader 四组渲染 + ⋯ 改组菜单（AppMenuButton，白名单/恢复自动）+ section/hero 接线 + CSS；启发式逐条 + 渲染/菜单钉测 + splitLessonFilesByRole 语义演进 |
