...
- V1.10.1（V1.10 最终验收前维护增量：移除资料选中重置竞态修复，D65）走查反馈修复使用单里程碑提交 `v1.10.1(V1101-A): <摘要>`（V173-fix / V18-fix 先例，不拆 plan 提交）；任务链位于 `implementation-tasks/v1.10.1-tasks/`（单节点 V1101-A）；零 migration、零新依赖、零新 IPC；验收证据追加至 `docs/v1.10-acceptance.md`（不改写 V110 历史记录），不创建 `checkpoint-V1.10.1-pass`——V1.5.3.1 先例，维护增量并入母版本验收，`checkpoint-V1.10-pass` 仍待产品负责人走查确认后创建。
