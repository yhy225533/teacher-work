...
- V1.9（课件导出可打印 PDF：隐藏打印窗 + printToPDF，D48–D54）方案提交使用 `plan(V1.9): <摘要>`，里程碑提交使用 `v1.9(V19-XX): <摘要>`；任务链位于 `implementation-tasks/v1.9-tasks/`；V19-C 为唯一全量验收点（全量测试、typecheck、lint、production build、diff check，不运行 portable/installer），产品负责人走查确认后创建 `checkpoint-V1.9-pass`（与既有 pass 标签互不替代、分别创建）；设计基准 `docs/v1.9-pdf-export-plan.md`，决策 `implementation-tasks/V1_9_DECISIONS.md` D48–D54；零 migration、零新依赖、零 AI 调用，新 IPC 仅 `export:print-to-pdf` / `export:get-print-payload` / `export:print-ready` 三条（载荷通道带 sender 校验）；PDF 不登记 files 表、不进索引/备份；与 V1.7/V1.7.2/V1.7.3/V1.8/V1.8.1 的待确认标签互不阻塞；实施待产品负责人完成 V1.8 / V1.8.1 走查确认后开始。
